...
class View:
    ...
    class MarkdownTabPanel(ui.tab_panel):
        ...
        def __init__(self, tab: ui.tab, doc_path: str, *args, **kwargs):
            super().__init__(*args, **kwargs)
    ...
    def _create_home_panel_version_1_0(self):
        with ui.tab_panel(self.tabs["主页"]).classes("temporary-custom-centered").classes(
            ...
            # todo: grid 似乎无法始终固定三列，不适合响应式页面开发
    ...
    def _create_moreitems_panel(self):
        self.MarkdownTabPanel(self.tabs["更多物品"], "./moreitems.md")

    def _create_moreitems_panel_version_1_0(self):
        ...
        with ui.tab_panel(self.tabs["更多物品"]):
            def create_moreitems_panel():
                ...
                with ui.row().classes("temporary-custom-centered").classes('w-full h-full gap-12'):
                    ...
                    #       实在不行，就隐藏左侧和右侧的内容吧！
                    # 左侧标题导航
                    with ui.column().classes("w-1/6 h-full bg-gray-100 p-4 rounded-lg overflow-y-auto hidden xl:block"):
                    ...
                    with ui.column().classes(
                            "w-1/6 h-full bg-gray-50 p-4 rounded-lg overflow-y-auto hidden xl:block"):
                        ui.label("页面导航").classes("text-xl font-bold mb-4")
                        ui.html(toc_html).classes("toc-container")

            create_moreitems_panel()

    def _create_petsenhancement_panel(self):
        self.MarkdownTabPanel(self.tabs["宠物增强"], "./petsenhancement.md")

    def _create_content(self):
        with ui.tab_panels(self.nav_tabs).classes("w-full"):
            self._create_home_panel()
            self._create_moreitems_panel()
            self._create_petsenhancement_panel()
    ...
    def _create_header_version_1_0(self):
